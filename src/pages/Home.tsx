...
const Home = () => {
	...
	const handleSearch = async () => {
		...
		try {
			...
			console.log(results);
			const convertedResults = results.map((result: { mapy: number; mapx: number }) => ({
				...result,
				...
				<div className="flex">
					<nav className="bg-darkGray w-24 h-screen items-center pb-2 flex flex-col justify-between">
						...
						<div className="flex flex-col w-full items-center pb-5">
							<Button variant="creme" size="md" className="w-16 h-14 text-[0.85rem] text-bold my-3">
								투표하기
							</Button>
							<Button variant="creme" size="md" className="w-16 h-14 text-[0.85rem] text-bold my-3">
								링크공유
					...
					<div
						...
						{MockProfiles.map(user => (
							<Button key={user.id} variant="creme" size="md" className={`w-14 h-14 bg-${user.bgColor}`}>
								{user.name}
							</Button>
						))}
					</div>
							...
							<ul>
								{searchResults.map((result, index) => (
									<li
										...
										<p className="text-[0.8rem] text-center">{result.address}</p>
									</li>
								))}
							</ul>
						</div>
